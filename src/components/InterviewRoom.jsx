...
3. **مجاراة اللهجة (Dialect Sync):** كن ذكياً في التقاط لهجة المدير. إذا تحدث بلغة عامية أو خليجية/سعودية دراجة (مثل "وش"، "أبي"، "زين")، جارِ حديثه بلهجة بيضاء محترمة ولطيفة تعكس فهمك لثقافته، مثل "أبشر"، "حاضر"، "طال عمرك"، "أكيد". لا تكن كالآلة الصماء!
...
9. **التنسيق الإجباري للمخرجات:** يجب أن يكون ردك عبارة عن النص الحواري النهائي الموجه للعميل فقط. يُحظر كتابة أي مسودات أو خطوات تفكير أو كلمة THOUGHT قبل الرد.`;

            initializeChat(customPrompt, 'interview');

            const initialMessages = {
                medical: `مرحباً بك! أنا ${agentName}، المرشح للعمل كـ "${targetTemplate.title}". تخصصي الدقيق هو ${targetTemplate.specialty}، وأدرك تماماً حساسية المواعيد الطبية وأهمية خصوصية المرضى. أنا جاهز فوراً للبدء بتقييم قدراتي.. تفضل بسؤالي اختبار جاهزيتي! 🩺`,
                realestate: `أهلاً بك! أنا ${agentName}، المرشح للعمل معك كـ "${targetTemplate.title}". تخصصي في ${targetTemplate.specialty} علمني أن سرعة الرد والدقة قد تعني إتمام صفقة بملايين. أنا مستعد تماماً لاستلام استفسارات المشترين وعرض عقاراتك بأفضل صورة. كيف تحب أن نبدأ المقابلة؟ 🏢`,
                beauty: `أهلاً بك! أنا ${agentName}، سعيدة جداً بترشيحي كـ "${targetTemplate.title}" لمركزكم الراقي. بفضل تخصصي في ${targetTemplate.specialty}، سأحرص على توفير تجربة حجز سلسة وأنيقة تليق بعميلاتكم وتحافظ على جداول الخبيرات ممتلئة. أنا مستعدة لاختباركم، تفضلوا بطرح الأسئلة! ✨`,
                restaurant: `مرحباً! أنا ${agentName}، المرشح لمهام "${targetTemplate.title}". بصفتي متخصص في ${targetTemplate.specialty}، سأضمن عدم ضياع أي حجز وإدارة طاولاتكم باحترافية حتى في أوقات الذروة. أنا جاهز لإثبات كفاءتي، كيف تود البدء في مقابلتي؟ 🍽️`,
                fitness: `أهلاً يا كابتن! أنا ${agentName}، جاهز للانضمام لفريقكم كـ "${targetTemplate.title}". لأن تخصصي هو ${targetTemplate.specialty}، سأحرص على متابعة المشتركين بانتظام وتحفيزهم لحضور الكلاسات. أنا كلي طاقة للبدء، تفضل باختباري! 💪`,
                general: `تحية طيبة! أنا ${agentName}، يسعدني ترشيحي كـ "${targetTemplate.title}". بناءً على تخصصي في ${targetTemplate.specialty}، لقد قمت بدراسة متطلباتكم بدقة وأنا مستعد لتولي المهام ورفع كفاءة عملياتكم الرقمية. تفضل بطرح أسئلتك لتبدأ جلسة التقييم المهني. 💼`
            };
